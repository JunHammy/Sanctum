...
// Resolves every relative-path <img> under `container` to a Drive blob
// URL. Extracted from the hook below so useTransclusion can call it
// directly on a transcluded note's freshly-injected content — that HTML
// lands via a raw `element.innerHTML = ...` mutation *outside* React's
// render cycle entirely, so the hook's own per-render effect (already run
// and finished by the time an embed's async fetch completes) never gets a
// chance to see those images at all. Returns a cancel function, same
// contract as an effect cleanup.
export function resolveImagesIn(container: Element, fileTree: FileTreeNode[], isVaultLoading: boolean): () => void {
  let cancelled = false
  ...
  images.forEach((img) => {
    ...
    if (isVaultLoading) {
      ...
      // refresh to ever show up. Show a spinner and let the next call
      // (the hook re-runs on every render) pick it up once loading finishes.
      showSpinner(img)
      return
    }
...
export function useImageResolution(
  ...
  useEffect(() => {
    ...
    // the returned cancel guards a stale first-pass fetch from writing
    // into a DOM node after this run's already been torn down.
    return resolveImagesIn(container, fileTree, isVaultLoading)
